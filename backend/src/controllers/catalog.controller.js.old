...
// ══════════════════════════════════════════════════════════════
// IMAGE UPLOAD (base64, no multer needed)
// ══════════════════════════════════════════════════════════════
 ...
 * Body: { type: 'header' | 'icon', filename: 'val.webp', data: '<base64>' }
 * Saves to frontend/public/images/{header|games_icon}/
 */
exports.uploadImage = async (req, res) => {
  try {
    ...
    if (!['.jpg', '.jpeg', '.png', '.webp', '.gif'].includes(ext)) {
      return res.status(400).json({ success: false, message: 'Ekstensi file tidak didukung. Gunakan jpg, png, webp.' });
    }

    // Strip base64 header if present (e.g. "data:image/webp;base64,...")
    ...
    res.json({
      success:  true,
      filename: safeName,
      url:      `/images/${folderMap[type]}/${safeName}`,
      message:  `File ${safeName} berhasil diupload`,
    });
  } catch (err) {
