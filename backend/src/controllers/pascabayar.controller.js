 ...
 *   2. POST /api/pascabayar/pay      → buat order + proses Duitku (TANPA hit Digiflazz dulu)
 *   3. GET  /api/pascabayar/inquiry/:refId → ambil data inquiry tersimpan
 *   4. [Duitku webhook sukses]        → baru hit Digiflazz pay-pasca (via duitku.controller)
 * 
 * Catatan Digiflazz:
 ...
 *   2. Validasi voucher (opsional)
 *   3. Hitung fee payment method
 *   4. Buat order di tabel orders (status: pending)
 *   5. Buat payment di Duitku → customer bayar
 *   6. [Duitku webhook] → Digiflazz pay-pasca dipanggil setelah pembayaran sukses
 */
exports.pay = async (req, res) => {
  ...
  try {
    ...
    // Link inquiry ke order — status tetap 'pending' sampai Duitku webhook sukses
    await client.query(`
