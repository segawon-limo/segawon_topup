...
// ── TOGGLE show_in_popup ─────────────────────────────────────
exports.togglePopup = async (req, res) => {
  try {
    const { id } = req.params;
    // Hanya 1 voucher aktif di popup — matikan yang lain dulu
    await pool.query(
      `UPDATE vouchers SET show_in_popup = FALSE, updated_at = NOW() WHERE show_in_popup = TRUE AND id != $1`,
      [id]
    );
    const result = await pool.query(
      `UPDATE vouchers SET show_in_popup = NOT show_in_popup, updated_at = NOW()
       WHERE id = $1 RETURNING id, code, show_in_popup`,
      [id]
    );
    if (!result.rows.length) return res.status(404).json({ success: false, message: 'Voucher tidak ditemukan' });
    res.json({ success: true, voucher: result.rows[0] });
  } catch (err) {
    console.error('togglePopup error:', err);
    res.status(500).json({ success: false, message: 'Gagal toggle popup' });
  }
};
...
// ── GET promo popup (public endpoint) ────────────────────────
exports.getPromoPopup = async (req, res) => {
  try {
    ...
    console.error('getPromoPopup error:', err);
    res.status(500).json({ success: false, message: 'Gagal mengambil promo popup' });
  }
};