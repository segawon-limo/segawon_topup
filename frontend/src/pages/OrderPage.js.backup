    ...
    if (productType === 'token_pln') {
      try {
        ...
        if (data.success) {
          ...
          setPlnCheckMethod(data.method || null);
          ...
          <div className="order-form">
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-section">
                ...
                {(() => {
                  const bundleProducts  = products.filter(p => p.section === 'bundle');
                  const generalProducts = products.filter(p => p.section !== 'bundle');
                  const hasSections     = bundleProducts.length > 0;

                  const renderGrid = (list) => list.map(product => {
                  ...
                  if (!hasSections) {
                    return <div className="products-grid">{renderGrid(generalProducts)}</div>;
                  }

                  return (
                    <>
                      <div className="products-section-label">{game?.section_bundle_label  || '🎁 Bundle & Membership'}</div>
                      <div className="products-grid products-grid--bundle">
                        {renderGrid(bundleProducts)}
                      </div>
                      <div className="products-section-label">{game?.section_general_label || '💎 Diamond & Top Up'}</div>
                      <div className="products-grid">
                        {renderGrid(generalProducts)}
                      </div>
                    </>
                  );
                })()}
              ...
              {selectedProduct && showStep2 && (
                <div className="form-section">
                  ...
                  {!userIdValidated && (
                    ...
                    >
                      {validating
                        ? (productType === 'token_pln' ? '🔍 Mengecek... (maks 30 detik)' : 'Memverifikasi...')
