...
const EWALLET_METHODS = [
  { code: 'OV', name: 'OVO',       logo: '/images/ovo-logo.png',       feeValue: 3.03 },
...
function PascabayarPage() {
  ...
  const handlePay = async () => {
    ...
    if (!phone.trim())          { setPayError('Nomor HP wajib diisi'); return; }
  ...
  const resetToInquiry = () => {
    ...
    setCustomerNo(''); setSelectedPaymentMethod('');
    handleRemoveVoucher(); setPayError('');
    setEmail(''); setPhone(''); setCustName('');
  };

  const isFormValid = () => email.trim() && phone.trim() && selectedPaymentMethod;
        ...
        {step === 'inquiry' && (
          <div className="pb-inquiry-wrap">
            <div className="order-form">
              ...
              {inquiryError && !selectedProduct && (
                <div className="error" style={{ marginTop: 12 }}>{inquiryError}</div>
              )}
            </div>
          </div>
        )}
          ...
          <div className="order-layout">
            ...
            <div className="order-form">
              ...
              <div className="form-section">
                <h2>1. Data Kontak</h2>
                <div className="form-group">
                  <label>Email <span className="pb-required">*</span></label>
                ...
                <div className="form-group">
                  ...
                  <input type="text" placeholder="Nama kamu (opsional)"
                    value={custName} onChange={e => setCustName(e.target.value)} />
                </div>
                <div className="form-group">
                  <label>Nomor HP <span className="pb-required">*</span></label>
              ...
              <div className="form-section">
                ...
                <div className="payment-methods">
                  <div className="payment-category">
                    ...
                    {VA_BANKS.map(b => (
                      <label key={b.code}
                        className={`payment-option ${selectedPaymentMethod === b.code ? 'selected' : ''}`}>
                        <input type="radio" name="payment" value={b.code}
                          checked={selectedPaymentMethod === b.code}
                          onChange={() => { setSelectedPaymentMethod(b.code); setPayError(''); }} />
                        <div className="payment-info">
                          <img src={b.logo} alt={b.name} className="payment-logo"
                            onError={e => { e.target.style.display='none'; }} />
                          <span className="payment-name">{b.name}</span>
                        </div>
                      </label>
                    ))}
                  </div>
              ...
              {/* Voucher — setelah metode pembayaran */}
