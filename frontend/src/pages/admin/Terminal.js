...
export default function AdminTerminal() {
  ...
  const [confirmCmd,   setConfirmCmd]   = useState(null); // { key, label, msgType, payload }
  const [inputPanel,   setInputPanel]   = useState(null); // { key, ...def, value:'' }
  ...
  // Klik tombol input command (psql / rm)
        ...
        <div className="term-sidebar">
          {sortedGroups.map(([group, items]) => (
            <div key={group} className="cmd-group">
              <div className="cmd-group-label">{GROUP_ICONS[group] || '⚡'} {group}</div>
              {items.map(cmd => (
                <button
                  key={cmd.key}
