...
CREATE TABLE products (
    ...
    selling_price DECIMAL(10,2) NOT NULL, -- Harga jual ke customer (legacy, use selling_price_qris)
...
CREATE TABLE orders (
    ...
    payment_fee DECIMAL(10,2) DEFAULT 0, -- Payment gateway fee
    subtotal DECIMAL(10,2), -- Before discount
    promo_code VARCHAR(50), -- Promo code used
    promo_discount DECIMAL(10,2) DEFAULT 0, -- Discount from promo
    discount_amount DECIMAL(10,2) DEFAULT 0, -- Total all discounts
    total_amount DECIMAL(10,2) NOT NULL, -- Final amount to pay
    
    -- Payment
    payment_method VARCHAR(100), -- 'qris', 'va', 'ewallet'
    payment_channel VARCHAR(100), -- 'bca_va', 'gopay', dll
    payment_gateway VARCHAR(50), -- 'midtrans', 'xendit', 'duitku'
    gateway_fee DECIMAL(10,2), -- Actual gateway fee
...
-- View untuk dashboard statistics (updated with promo info)
...
COMMENT ON COLUMN orders.promo_code IS 'Promo code used in this order';
COMMENT ON COLUMN orders.promo_discount IS 'Discount amount from promo code';

COMMENT ON DATABASE topup_game IS 'Database untuk sistem instant topup game dengan promo code support';
